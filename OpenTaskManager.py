...
class ViewTaskIn(Qt.QWidget):
    ...
    def create_1(self, text: str, level: int, is_checked: bool, row: int, date: datetime.datetime):
        ...
        self.tasks.append([date, text, level, is_checked])
    ...
    def onCellChanged(self, row, column):
        ...
        if currentState != lastState:
            if currentState == QtCore.Qt.Checked:
                print(f'now checked in {row}')
            else:
                print(f'now unchecked in {row}')
...
class TaskManager(QtWidgets.QMainWindow):
    ...
    def __init__(self, date = None, newExemplar = None, tasks = None, *args, **kwargs):
        ...
        container = ViewTaskIn(newExemplar)
        ...
        if tasks != None:
            ...
            for j in range(len(tasks)):
                print('ura')
                container.create_1(tasks[j]['note'], tasks[j]['grade'], tasks[j]['flag'], i, date)
                i += 1
        if date:
